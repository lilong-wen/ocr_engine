...
def getDetBoxes_core(textmap, linkmap, text_threshold, \
    ...
    nLabels, \
        labels, \
        stats, \
        centroids = cv2.connectedComponentsWithStats(text_score_comb.astype(np.uint8), \
                                                     connectivity=4)
    ...
    for k in range(1,nLabels):
        ...
        if estimate_num_chars:
            _, character_locs = cv2.threshold((textmap - linkmap) * segmap /255., \
                                              text_threshold, 1, 0)
